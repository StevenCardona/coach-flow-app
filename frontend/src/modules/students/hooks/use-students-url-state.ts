...
export function useStudentsUrlState() {
  ...
  const isCreateOpen = pathname.endsWith("/students/new");
  const editStudentId = pathname.includes("/edit") ? (params.id ?? "") : "";
  const isEditOpen = Boolean(editStudentId);
  ...
  const updateFilters = React.useCallback(
    (updates: Partial<PaginationParams> & { statusFilter?: StatusFilterValue }) => {
      ...
      router.push(query ? `${pathname}${query ? `?${query}` : ""}` : pathname);
    },
    [pathname, router, searchParams],
  );

  const openCreate = React.useCallback(() => {
    router.push(`/students/new${filterQuery}`);
  }, [filterQuery, router]);

  const openEdit = React.useCallback(
    (studentId: string) => {
      router.push(`/students/${studentId}/edit${filterQuery}`);
    },
    [filterQuery, router],
  );

  const closeAction = React.useCallback(() => {
    router.push(`/students${filterQuery}`);
  }, [filterQuery, router]);
